...
sweep_config = {
    ...
    'parameters': {
        ...
        'learning_rate': {
            'max': 0.00005, 'min': 0.00002 #'values': [ 5e-5, 3e-5, 2e-5]
        },
        'batch_size': {
            'values': [8]
        },
        'epochs':{
            "max": 14, "min": 4 #'values':[1, 2, 3]
        }
    }
}
...
sweep_id = wandb.sweep(sweep_config, project="LREC-COLING Bias")
...
def train():    # train_data, train_tags
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        einput_ids = tokenizer(train_data, padding=True, truncation=True, max_length = 512, return_tensors='pt')
        input_ids, attention_mask = einput_ids.input_ids, einput_ids.attention_mask
        labels = tokenizer(train_tags, padding=True, truncation=True, max_length = 512, return_tensors='pt').input_ids
        ...
        wandb.log({'lr_rate':wandb.config.learning_rate})
        wandb.log({'epoch':wandb.config.epochs})
...
def evaluate(val_data, val_tags, model, tokenizer, device):
    ...
    # tokenizer.encode() converts the text to a list of unique integers before returning tensors
    einput_ids = tokenizer(val_data, padding=True, truncation=True, max_length = 512, return_tensors='pt')
    input_ids, attention_mask = einput_ids.input_ids, einput_ids.attention_mask
    labels = tokenizer(val_tags, padding=True, truncation=True, max_length = 512, return_tensors='pt').input_ids
